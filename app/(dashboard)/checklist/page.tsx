...
export default function ChecklistPage() {
  ...
  async function handleToggle(templateId: Id<"checklist_templates">) {
    await toggleCompletion({ templateId, dateKey: selectedDate });
  }

  async function handleAddTemplate() {
    if (!newLabel.trim()) return;
    const allTemplates = templates ?? [];
    const maxOrder = allTemplates.reduce((m, t) => Math.max(m, t.order), -1);
      ...
      <div className="flex items-center justify-between mb-6">
        <div>
          ...
          <p className="text-zinc-500 text-sm mt-1">
            Tarefas recorrentes do painel de LED
          </p>
        </div>
        ...
        <>
          ...
          {templates === undefined || completions === undefined ? (
            ...
            <EmptyState
              ...
              description="Adicione tarefas recorrentes para acompanhar o que precisa ser feito no painel."
            ...
            return (
              <Card key={dateKey} padding={false}>
                <div className="px-5 py-4">
                  ...
                  {dayCompletions.length > 0 && templates && (
                    <p className="text-xs text-zinc-400 mt-2">
                      {dayCompletions.length} de {templates.length} tarefas
                    </p>
                  )}
